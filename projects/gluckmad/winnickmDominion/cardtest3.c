...
int main() {
    ...
    printf ("\n\nTESTING cardEffect() with Great Hall:\n");
    for (numPlayer = 2; numPlayer <= MAX_PLAYERS; numPlayer++)
    {
        for (p = 0; p < numPlayer; p++)
        {
            ...
            cardEffect(great_hall, 0, 0, 0, &G, handPos, NULL);
            
            // Count cards in hand and deck after
            ...
            if (handBefore == handAfter && actionsBefore == actionsAfter - 1) // check that hand and actions change correctly
            {
                printf("\tPASS\n");
                passCount++;
            }
