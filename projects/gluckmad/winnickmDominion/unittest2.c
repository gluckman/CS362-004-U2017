...
int main() {
    ...
    {
#if (NOISY_TEST == 1)
        printf("Test %d-player game with Province pile depleted.\n", numPlayer);
#endif
        memset(&G, 23, sizeof(struct gameState));   // clear the game state
        initializeGame(numPlayer, k, seed, &G); // initialize a new game
        G.supplyCount[province] = 0; // Set Province pile to depleted
        r = isGameOver(&G);
#if (NOISY_TEST == 1)
        ...
        {
            for (j = 0; j < sizeof(G.supplyCount)/sizeof(G.supplyCount[0]); j++)
            {
                for (m = 0; m < sizeof(G.supplyCount)/sizeof(G.supplyCount[0]); m++)
                {
                    if (i != j && j != m && m != i)
                    {
#if (NOISY_TEST == 1)
